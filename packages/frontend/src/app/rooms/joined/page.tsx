...
export default function JoinedRoomsPage() {
  ...
  useEffect(() => {
    ...
    const onMemberJoined = () => revalidate();
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      <div className="mb-6 rounded-2xl border border-neutral-800 bg-neutral-950 p-5 sm:p-6">
        <h1 className="text-xl font-semibold text-white sm:text-2xl">Joined</h1>
        <p className="mt-1 text-sm text-neutral-500">Rooms you&apos;re a member of.</p>
      </div>
      ...
      {isLoading ? (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-36 animate-pulse rounded-2xl border border-neutral-900 bg-neutral-950" />
          ))}
        </div>
      ) : rooms.length === 0 ? (
